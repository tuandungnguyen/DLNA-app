    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        deviceControl = DLNACastManager.connectDevice(device, object : OnDeviceControlListener {
            override fun onConnected(device: Device<*, *, *>) {
                Toast.makeText(requireContext(), "Connected successfully: ${device.details.friendlyName}", Toast.LENGTH_SHORT).show()
            }

            override fun onDisconnected(device: Device<*, *, *>) {
                (requireActivity() as MainActivity).onBackPressed()
                Toast.makeText(requireContext(), "Unable to connect: ${device.details.friendlyName}", Toast.LENGTH_SHORT).show()
            }
